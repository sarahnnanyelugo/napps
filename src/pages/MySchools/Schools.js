...
export const Schools = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const activeSub = data?.subscriptions.find(
      (subscription) =>
        subscription.expired === 0 && subscription.status === "successful"
    );
    if (activeSub) setActiveSubscription(activeSub);

    if (data && data.subscriptions) {
      const { number, max, remaining } = progressBarCalculation({
  ...
  return (
    <div style={{ padding: "5px" }}>
      ...
      >
        {" "}
        <div className="schools d-md-flex">
          {isLoading ? (
            <Skeleton
              count={1}
              style={{ width: "200px" }}
          ...
          <div className="sch-details  d-md-flex ">
            <div className=" justify-content-between align-items-center">
              <div className="ditss">
                ...
                <p>
                  <img
                    src={data.status ? getGreen() : getRed()}
                    style={{ width: "10px" }}
                  />
              ...
              {!isLoading && (
                <p>
                  <ProgressBar
                    number={number}
                    max={max}
                    color={remColor(remaining)}
                  />
                  <p>
                    Remaining days: <strong>{remaining}</strong>
                  </p>
                </p>
              )}
            </div>
