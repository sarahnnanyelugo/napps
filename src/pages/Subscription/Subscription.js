...
export const Subscription = () => {
    ...
    async function fetchSubscriptions() {
        try {
            ...
            setActiveSubscriptions(data.subscriptions.filter(sub => sub.status === 'successful'));
            setInactiveSubscriptions(data.subscriptions.filter(sub => sub.status !== 'successful'));
        } catch (errorResponse) {
            ...
            <div className="Admin-dashboard">
                <div className="row row-cols-1 row-cols-lg-3 g-2 g-lg-4 mt">
                    ...
                    <div className="col">
                        <div className="summary d-flex">
                            <div style={{flexGrow: 1}}>
                                <img src={Icon2} height="30px" alt="Active Subscriptions Icon"/>
                    ...
                    <div className="col">
                        <div className="summary d-flex">
                            <div style={{flexGrow: 1}}>
                                <img src={Icon2} height="30px" alt="Expired Subscriptions Icon"/>
                ...
                <div className="schools-filter-div col-md-12">
                    ...
                    <div className="d-flex justify-content-start mb-3">
                        <Button variant="primary" onClick={() => setFilter('total')}>Total</Button>
                        <Button variant="success" onClick={() => setFilter('active')}>Active</Button>
                        <Button variant="danger" onClick={() => setFilter('inactive')}>Inactive</Button>
                    </div>
                    ...
                    <Table striped bordered hover>
                        ...
                        {filteredData.map(sub => (
                            <tr key={sub.id}>
                                <td><img src={sub.school.logo} alt="Logo" width="50"/></td>
                                <td>{sub.school.name}</td>
                                <td>{sub.proprietor.name}</td>
                                <td>{sub.tx_ref}</td>
                                <td>{formatNumber(sub.amount)}</td>
                                <td>{sub.start_date} - {sub.end_date}</td>
                                <td><img src={sub.status==='successful'?getGreen():getRed()} alt="status icon" style={{width:'10px'}}/> {sub.status?sub.status:'Pending'}</td>
                                <td>
                                    <Link
                                        className="view"
